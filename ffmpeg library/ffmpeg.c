...
float gDesiredBitRate; //added ARC should be 64 if not existing
void setDesiredBitRate (float newBitrate)
{
    gDesiredBitRate = newBitrate;
}
...
{
    return  gStopFFMPEG ||  q_pressed || (q_pressed = read_key() == 'q');
}

static int av_exit(int ret)
{
    int i;
	fprintf(stderr,"av_exit ret=%d\n",ret);
    /* close files */
...
char old_title[512];
char old_author[512];
char old_copyright[512];
char old_comment[512];
char old_album[512];
char old_genre[512];
void ffmpeg_stream_info_callback(char *songtitle, char *author, char *copyright, char *comment, char *album, char *genre, int year, int track );

static void send_title(AVFormatContext *is)
{
	
	if (
		strcmp(old_title,is->title) ||
		strcmp(old_author,is->author) ||
		strcmp(old_copyright,is->copyright) ||
		strcmp(old_comment,is->comment) ||
		strcmp(old_album,is->album) ||
		strcmp(old_genre,is->genre) )
	{
		ffmpeg_stream_info_callback(
						  is->title,
						  is->author,
						  is->copyright,
						  is->comment,
		...
		strcpy(old_title,is->title);
		strcpy(old_author,is->author);
		strcpy(old_copyright,is->copyright);
		strcpy(old_comment,is->comment);
		strcpy(old_album,is->album);
		strcpy(old_genre,is->genre);
...
if (is->title[0])
{
	fprintf(stderr,"title [%s] %d %d",is->title,is->year,is->track);
}
if (is->author[0]) fprintf(stderr,"title [%s]",is->author);
if (is->copyright[0]) fprintf(stderr,"copyright [%s]",is->copyright);
if (is->comment[0]) fprintf(stderr,"comment [%s]",is->comment);
if (is->album[0]) fprintf(stderr,"album [%s]",is->album);
if (is->genre[0]) fprintf(stderr,"genre [%s]",is->genre);
#endif
                     ...
                     AVStreamMap *stream_maps, int nb_stream_maps)
{
    ...
    for(i=0;i<nb_output_files;i++) {
        os = output_files[i];
        if (!os->nb_streams) {
			Status_SetNewStatusByKey("ERROR_DECODING_STREAM");
    ...
    for(i=0;i<nb_stream_maps;i++) {
        ...
        if (fi < 0 || fi > nb_input_files - 1 ||
			...
			//Status_SetNewStatusString("Error decoding stream - input");
			Status_SetNewStatusByKey("ERROR_DECODING_STREAM");
        ...
        if (fi < 0 || fi > nb_input_files - 1 ||
            si < 0 || si > file_table[fi].nb_streams - 1) {
			//Status_SetNewStatusString("Error decoding stream - sync");
			Status_SetNewStatusByKey("ERROR_DECODING_STREAM");
            ...
            if (nb_stream_maps > 0) {
                ...
                if(opt_programid) {
                    ...
                    for(j=0;j<nb_istreams;j++) {
                        ist = ist_table[j];
                        if (ist->discard &&
                            ist->st->codec->codec_type == ost->st->codec->codec_type/*  && ist->st->codec->codec_id!=CODEC_ID_NONE && ist->st->codec->codec!=NULL*/ /* AJS - try to ignore things with invalid codecs */) {
							
							// AJS 
							if (!found)
							{
								ost->source_index = j;
								lastbitrate= ist->st->codec->bit_rate;
								found = 1;
							}
							else 
							{
								...
								fprintf(stderr," lastrate %d new rate %d\n",lastbitrate,ist->st->codec->bit_rate);
								double thisdif = (gDesiredBitRate*1024.0 - ist->st->codec->bit_rate) *  (gDesiredBitRate*1024.0 - ist->st->codec->bit_rate);
								double lastdif = (gDesiredBitRate*1024.0 - lastbitrate) * (gDesiredBitRate*1024.0 - lastbitrate);
								fprintf(stderr," lastdif %f thisdif %f NEWSTREAM: %d\n",lastdif,thisdif,j);
								if ( thisdif < lastdif)
								{
...
/* AJS */	
	for(i=0;i<nb_ostreams;i++) {
		ost = ost_table[i];
		//ist_table[ost->source_index]->discard=AVDISCARD_DEFAULT;
		//is = input_files[ist->file_index];
		//is[
		fprintf(stderr,"source index: %d\n",ost->source_index);
		is->streams[ost->source_index]->discard=AVDISCARD_DEFAULT;
	}
    ...
    for(i=0;i<nb_istreams;i++) {
        ...
        if (ist->decoding_needed) {
            ...
            if (!codec) {
				//Status_SetNewStatusString("Unsupported codec for input stream");
				Status_SetNewStatusByKey("UNSUPPORTED_CODEC");
                fprintf(stderr, "Unsupported codec (id=%d) for input stream #%d.%d\n",
                        ist->st->codec->codec_id, ist->file_index, ist->index);
                av_exit(1);
            }
            if (avcodec_open(ist->st->codec, codec) < 0) {
				//Status_SetNewStatusString("Error opening codec for input stream");
				Status_SetNewStatusByKey("UNSUPPORTED_CODEC");
    ...
    if (!using_stdin && verbose >= 0) {
		...
		// AJS - hook in the quit code here!!!!!
    ...
    redo:
		...
		//
		// check to see if we should send a new title...
		//
		send_title(is);
...
//		fprintf(stderr,"pkt.stream_index %d nb_streams %d\n",pkt.stream_index,file_table[file_index].nb_streams);
        ...
        if (ist->discard)
		{
			...
			snprintf(message,256,"discard pkt (good %d discard %d) %d",good_packet,discard_packet,ist_index);
			Status_SetNewStatusString(message);
			if (good_packet<3 && discard_packet>10)
		//	if (good_packet<3 && discard_packet>1000)
			{
				gUseStreamNum=ist_index;
				av_exit(-1010);
			}
            goto discard_packet;
		}
...
//        fprintf(stderr, "next:%"PRId64" dts:%"PRId64" off:%"PRId64" %d\n", ist->next_pts, pkt.dts, input_files_ts_offset[ist->file_index], ist->st->codec->codec_type);
     ...
     //   fprintf(stderr,"read #%d.%d size=%d stream_index=%d\n", ist->file_index, ist->index, pkt.size,pkt.stream_index);
	//	if (pkt.size < 2230 && pkt.stream_index==0)
	//	{
	//		fprintf(stderr,"we have some kind of problem\n");
	//	}
        if (output_packet(ist, ist_index, ost_table, nb_ostreams, &pkt) < 0) {
			//Status_SetNewStatusString("Error while decoding stream");
			Status_SetNewStatusByKey("ERROR_WHILE_DECODING");
            if (verbose >= 0)
...
void audio_decode_example(const char *outfilename, const char *filename)
{
    ...
    codec = avcodec_find_decoder_by_name("wmav1");
	//CODEC_ID_WMAV1
	...
	fprintf(stderr,"c is %x\n",c);
    /* open it */
    if (res=avcodec_open(c, codec) < 0) {
        fprintf(stderr, "2 could not open codec %d %x\n",res,codec);
		//        exit(1);
    }
...
void setup_globals_ajs()
{
	...
	nb_meta_data_maps=0;
	 ...
	 frame_padleft  = 0;
	 frame_padright = 0;
	padcolor[0]=16;padcolor[1]=128;padcolor[2]=0;
	 frame_topBand  = 0;
	 frame_bottomBand = 0;
	 frame_leftBand  = 0;
	 frame_rightBand = 0;
...
int main(int argc, char **argv)
{
	...
	//    fmt = guess_format("foo.mp3", NULL, NULL);
    ...
    oc->oformat = fmt;
    strcpy(oc->filename,"mynewout.mp3");
	...
	res = av_open_input_file(&ic, "myfile.wma", NULL, 0,NULL);
	fprintf(stderr,"av_open_input_file %d\n",res);
	res = av_find_stream_info(ic);
	fprintf(stderr,"av_find_stream_info %d\n",res);
	dump_format(ic, 0, "myfile.wma", 0);
	
	AVPacket pkt;
	AVCodec *codec;
	FILE *outFile=NULL;
	outFile=fopen ("myrawfile","wb");
...
//		fwrite(pkt.data,pkt.size,1,outFile);
		...
		outpkt.size= avcodec_encode_audio(c, pkt.data, pkt.size, NULL);
	{
	...
	return 0;
	audio_decode_example("outputfile","myfile.wma");
