...
void ff_acelp_isp2lpc(int16_t *lp, const int16_t *isp, int lp_half_order)
{
	    int f1[lp_half_order+1];
	    int f2[lp_half_order  ];
	    int i;
	    int reflection_coeff = isp[(lp_half_order<<1)-1];
	
	    lsp2poly(f1, isp  , lp_half_order  );
	    lsp2poly(f2, isp+1, lp_half_order-1);
	    ...
	    for(i=0; i<lp_half_order; i++)
		    {
			        f1[i] = (((int64_t)f1[i]) * (32767 + reflection_coeff)) >> 15;
			        f2[i] = (((int64_t)f2[i]) * (32767 - reflection_coeff)) >> 15;
		    }
	    f1[lp_half_order] = (((int64_t)f1[lp_half_order]) * (32767 + reflection_coeff)) >> 15;
	
	    lp[0] = 4096;
	    for(i=1; i<lp_half_order; i++)
		    {
			        lp[i]                    = (f1[i] + f2[i]) >> 11;
			        lp[(lp_half_order<<1)-i] = (f1[i] - f2[i]) >> 11;
		    }
	    lp[lp_half_order     ] = f1[lp_half_order] >> 11;
	    lp[lp_half_order << 1] = reflection_coeff >> 3;
}
