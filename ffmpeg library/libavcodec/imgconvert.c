...
#include "../arm_yuv2rgb/yuv2rgb_arm.h"

static void yuv420p_to_rgb32_assembly(AVPicture *dest, const AVPicture *src,
								   int width, int height)
{
	//fprintf(stderr,"yuv420p_to_rgb32_assembly called\n");
	/*
	 *
	 * mb_YUV420_to_RGB32
...
//	int w2=dest->linesize[0];
	int size=(width << 16 | height);
//	int size=(w2 << 16 | height);
	char *yuv[3];
	yuv[0]=src->data[0];
	yuv[1]=src->data[1];
	yuv[2]=src->data[2];
	...
	{
		printf("B %0x ",dest->data[0][i]);
	}
	printf("\n");
#endif
	
	 mb_YUV420_to_RGB32( size, src->linesize[0], yuv, dest->data[0]);
#if 0
	for(int i=0;i<width*4;i++)
	{
		printf("A %0x ",dest->data[0][i]);
	}
	printf("\n");
#endif
	...
	for (int i=0;i< height;i++)
	{
		...
		{
			int pos=i*width*4 + width*4;
			dest->data[0][pos]=0xFF;
...
#if 0
static void yuv420p_to_rgb32_table(AVPicture *dest, const AVPicture *src,
								   int width, int height)
{
	asm volatile (
				  VFP_VECTOR_LENGTH(3)
...
static void yuv420p_torgb32_arthur(AVPicture *dst, const AVPicture *src,
								   int width, int height)
{
...
.convert = yuv420p_to_rgb32_assembly
#else
.convert = yuv420p_to_rgb32
#endif
        },
