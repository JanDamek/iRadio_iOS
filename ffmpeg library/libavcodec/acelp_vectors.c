									        ...
									        int bits)
{
	    ...
	    for(i=0; i<pulse_count; i++)
		    {
			        int index_1 = i + tab1[(pulse_indexes[i] >> bits) & mask];
			        int index_2 = i + tab2[ pulse_indexes[i]          & mask];
			        int pulse   = (pulse_indexes[i] & 0x100) ? -8192 : 8192;
			
			        fc_v[index_1] = pulse;
			        fc_v[index_2] += (index_2 <  index_1) ? -pulse : pulse;
		    }
}
