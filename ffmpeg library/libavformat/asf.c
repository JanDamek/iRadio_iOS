...
//                type = CODEC_TYPE_DATA;
				type = CODEC_TYPE_SUBTITLE; /* AJS SUBTITLE FIX */
...
//	fprintf(stderr,"asf->stream_index=%d\n",	asf->stream_index);
...
#if 0
			fprintf(stderr,"new packet: stream:%d key:%d packet_key:%d audio:%d size:%d\n",
asf->stream_index, asf->packet_key_frame, asf_st->pkt.flags & PKT_FLAG_KEY,
s->streams[asf->stream_index]->codec->codec_type == CODEC_TYPE_AUDIO, asf->packet_obj_size);
#endif		
			
			static int lastpackettype=-1;
			if (lastpackettype != CODEC_TYPE_AUDIO && 		s->streams[asf->stream_index]->codec->codec_type != CODEC_TYPE_AUDIO	)
			{
				// are we going to crash?
				fprintf(stderr,"are we going to crash?\n");
			}
			
			lastpackettype=s->streams[asf->stream_index]->codec->codec_type;
