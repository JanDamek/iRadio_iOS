...
// AJS
int rtsp_default_protocols = (1 << RTSP_LOWER_TRANSPORT_TCP);
#endif
                           ...
                           int letter, const char *buf)
{
    ...
    case 'm':
        ...
        } else if (!strcmp(st_type, "application")) {					/* AJS SUBTITLE FIX */
        ...
        /* put a default control url */
			if (strlen(rt->controlfilenamepath))
				av_strlcpy(rtsp_st->control_url, rt->controlfilenamepath, sizeof(rtsp_st->control_url));
			else
    ...
    case 'a':
			...
			{
				if (s->nb_streams == 0) {
				fprintf(stderr,"control url is: %s\n",p);
					// check to see if the url starts with rtsp
					if (!strncasecmp(p,"rtsp://",7))
						av_strlcpy(rt->controlfilenamepath, p, sizeof(rt->controlfilenamepath));
				}
				else// s->nb_streams > 0) {
				{
            char proto[32];
            ...
            url_split(proto, sizeof(proto), NULL, 0, NULL, 0, NULL, NULL, 0, p);
					...
					/* AJS END SUBTITLE FIX */
					
            if (proto[0] == '\0') {
                /* relative control URL */
				// AJS AJS AJS - remove // problem
			...
			// AJS
        } else if(av_strstart(p, "pgmpu:data:application/x-wms-contentdesc", &p)) {
			// pull apart the content description

#ifdef DEBUG
			p++;
			printf("pgmpu:data:application/x-wms-contentdesc:'%s'\n", p);
			...
			//	printf("len=%d p[%s]\n",len,p);
				// name
				char name[1024];
				if (len>sizeof(name)) break;
				av_strlcpy(name, p, len+1);
				p+=len+1;
				int value_type=atoi(p);
				while  (*p!=',' && *p!=0) p++;
				if (*p==0) break;
				p++;
				len = atoi(p);
			...
			//	printf("len=%d p[%s]\n",len,p);
				...
				while (*p!=',' && *p!=0) *v++=*p++;
				*v=0;
				//if (len>sizeof(value)) break;
				//av_strlcpy(value, p, len+1);
				//printf("AFTER VALUE p[%s]\n",p);

				if (*p==0) done=1;
				
				p++;
				
				printf("CONTENTDESC: name [%s] value[%s] value_type[%d]\n",name,value,value_type);
				if (!strncasecmp(name,"title",5))
				{
					// url decode the value
					unescape_url(value);
					av_strlcpy(s->title, value, sizeof(s->title));
				}
				if (!strncasecmp(name,"author",6))
				{
					// url decode the value
					unescape_url(value);
					av_strlcpy(s->author, value, sizeof(s->author));
				}
				if (!strncasecmp(name,"copyright",9))
				{
					// url decode the value
					unescape_url(value);
					av_strlcpy(s->copyright, value, sizeof(s->copyright));
				}
                          ...
                          unsigned char **content_ptr)
{
   ...
   if (reply) memset(reply, 0, sizeof(RTSPHeader));
	...
	// AJS AJS -- try not to parse the result here.. let's see what happens
    ...
    for(;;) {
        ...
        for(;;) {
			...
			//  --  AJS fix 104.6 RTL in Berlin, Germany (see first character comment)  -- 
            if (ch == '$' && linepos==0) {
                    ...
                    int lower_transport, const char *real_challenge)
{
	...
	if ( rt->server_type==RTSP_SERVER_WMS)
	{
		...
		fprintf(stderr,"AJS s->nb_streams %d\n",s->nb_streams);
		
	// CODEC_TYPE_AUDIO
	// CODEC_TYPE_VIDEO
	//	s->streams[r]->codec->codec_type==

		
	for (int r = 0; r < s->nb_streams; r++) 
	{
		
		if (s->streams[r]->codec->codec_type==CODEC_TYPE_AUDIO)
		{

		
			if (!found)
			{
				lastbitrate=s->streams[r]->codec->bit_rate;
				used=r;
				found=1;
			}
			else
			{
				double thisdif = (gDesiredBitRate*1024.0 - s->streams[r]->codec->bit_rate) *  (gDesiredBitRate*1024.0 - s->streams[r]->codec->bit_rate);
				double lastdif = (gDesiredBitRate*1024.0 - lastbitrate) * (gDesiredBitRate*1024.0 - lastbitrate);
				if ( thisdif < lastdif)
				{
		...
		else if (s->streams[r]->codec->codec_type==CODEC_TYPE_VIDEO)
		{
			fprintf(stderr,"this is a video stream %d\n",r);
			if (gTurnOnVideo)
			{
			if (!vfound)
			{
		...
		/* AJS SUBTITLE FIX */
		else  if (s->streams[r]->codec->codec_type==CODEC_TYPE_SUBTITLE)
		{
			fprintf(stderr,"SUBTITLE CODEC TYPE\n");
			sused=r;
		}
		...
		for (int r = 0; r < s->nb_streams; r++) 
		{			
			if (r==used || r==vused || r==sused)		/* AJS SUBTITLE FIX */

			{
				s->streams[r]->discard = AVDISCARD_DEFAULT;
			}
			else
			{
				s->streams[r]->discard = AVDISCARD_ALL;
			}
		}
	}		
    ...
    for(j = RTSP_RTP_PORT_MIN, i = 0; i < rt->nb_rtsp_streams; ++i) {
		...
		if (rt->server_type==RTSP_SERVER_WMS)
		{
		fprintf(stderr,"AJS - this rtsp stream we are checking is number %d\n",i);
		...
		for (int r = 0; r < s->nb_streams; r++) 
		{
			...
			{
				fprintf(stderr,"AJS - this stream is turned off %d bitrate %d \n",r, s->streams[r]->codec->bit_rate);
			}
			else
			{
				fprintf(stderr,"AJS - this stream is turned on %d bitrate %d\n",r, s->streams[r]->codec->bit_rate);
			...
			if (s->streams[r]->priv_data == rt->rtsp_streams[i]) 
			{
				if (s->streams[r]->discard == AVDISCARD_ALL) 
				{
					fprintf(stderr,"AJS - skipping stream %d \n",i);
					skip =1;
				}
			}
		}
...
*/
        ...
        /* RTP/TCP */
            ...
            if (s->streams[rtsp_st->stream_index]->codec->codec_type == CODEC_TYPE_DATA)
                continue;
        }
        ...
        if (k == 0 && rt->server_type == RTSP_SERVER_REAL) {
	...
	if (rt->server_type == RTSP_SERVER_REAL)
	{
		snprintf(cmd, sizeof(cmd),
				 ...
				 rtsp_st->control_url);
		av_strlcat(cmd,
				   "SetDeliveryBandwidth: Bandwidth=512000;BackOff=0\r\n",
				   sizeof(cmd));
		...
		{
			// we don't really care if we get an error here..
		}
...
/*	
	if (rt->server_type==RTSP_SERVER_WMS)
	{
		fprintf(stderr,"WMS Server: state = %d\n",rt->state);
		rt->need_subscription=1;
		return 0;
	}
 */
...
static int rtsp_setup_and_play(AVFormatContext *s)
{
	RTSPState *rt = s->priv_data;
	int port,  err;
	int lower_transport_mask = (1<< RTSP_LOWER_TRANSPORT_TCP);
	...
	do {
        ...
        err = make_setup_request(s, host, port, lower_transport,NULL);
        if (err < 0)
            goto fail;
        lower_transport_mask &= ~(1 << lower_transport);
        if (lower_transport_mask == 0 && err == 1) {
...
#ifdef DEBUG_RTP_TCP
        printf("OTHER STUFF?:: ret=%d c=%02x [%c]\n", ret, buf[0], buf[0]);
#endif
		
		//
		// IF WE ARE HERE, WE HAVE A PACKET THAT IS PROBABLY AN RTSP FRAME
		//
		char bufa[4096], buf1[1024], *q;
...
//if (ch == '$') {
			...
			*q = '\0';
			...
			if (line_count == 0) {
				...
				//
				// we need to look at what we got, and pull out some important things, like content length
				//
				//Content-Length: 42
				...
				{
					skip_spaces(&p);
					av_strlcpy(XNotice, p, sizeof(XNotice));
				}
		...
		// NOW WE NEED TO PULL THE BODY OUT IF THERE IS A BODY - CHECK THE CONTENT LENGTH
		if (content_length>0)
		{
			char *buf3=av_mallocz(content_length);
			if (url_readbuf(rt->rtsp_hd, buf3, content_length) != content_length)
			{
				// error??
				av_free(buf3);
			}
		...
		//
		//  We pulled out the header and the body..
		//
		
		// let's check for WMS SET_PARAMETER with a header of X-Notice: 2101
		if (av_stristart(cmd, "SET_PARAMETER", &p))
		{
			// check to see the X-Notice
			int number = strtol(XNotice, NULL, 10);
			if (number==2101)
			{
				return -1;
			}
...
//    if (len > buf_size || len < 12)
  //      goto redo;
    ...
    for(i = 0; i < rt->nb_rtsp_streams; i++) {
        rtsp_st = rt->rtsp_streams[i];
        if (rtsp_st->tx_ctx!=NULL &&  id >= rtsp_st->interleaved_min &&
                            ...
                            AVPacket *pkt)
{
	...
	//fprintf(stderr,"rtsp_read_packet\n");

	
	
#if 0
	if (rt->server_type==RTSP_SERVER_WMS && rt->need_subscription ==1)
	{
		...
		ret= rtsp_setup_and_play(s);
	}
#endif
#if 0
	if (rt->server_type==RTSP_SERVER_WMS && rt->need_subscription ==0)
	{
		/* check out cache and see if things have changed */
        enum AVDiscard cache[MAX_STREAMS];
        char cmd[1024];
		RTSPHeader reply1, *reply = &reply1;
       for (int i = 0; i < s->nb_streams; i++)
		...
		if (memcmp (cache, rt->real_setup_cache,
					sizeof(enum AVDiscard) * s->nb_streams)) {
			fprintf(stderr,"AJS - we have a change in streams, let's try to teardown and resetup\n");
			snprintf(cmd, sizeof(cmd),
					 "TEARDOWN %s RTSP/1.0\r\n",
					 s->filename);
			rtsp_send_cmd(s, cmd, reply, NULL);
...
//{
//			rtsp_send_cmd(s, "OPTIONS * RTSP/1.0\r\n", NULL, NULL);
...
//		}
	...
	if ((av_gettime() - rt->last_cmd_time) / 1000000 >= rt->timeout / 2) 
	{
		...
		if (rt->server_type==RTSP_SERVER_WMS)
		{
			...
			if (rt->controlfilenamepath[0]!=0)
			{
				playurl=rt->controlfilenamepath;
			}
			else
			{
				playurl=s->filename;
			...
			snprintf(cmd, sizeof(cmd),
					 ...
					 playurl);
			// AJS			rtsp_send_cmd(s, cmd, reply, NULL);
						rtsp_send_cmd(s, cmd, NULL, NULL);
 ...
 redo:
	...
	{
		if (rtsp_st->tx_ctx==NULL) return 0;
...
static int rtsp_read_play(AVFormatContext *s)
{
    ...
    if (!(rt->server_type == RTSP_SERVER_REAL && rt->need_subscription)) {
		...
		{
			av_strlcat(cmd,
					 //  "Bandwidth: 1685936\r\nX-Accelerate-Streaming: AccelBandwidth=2147483647;AccelDuration=28000\r\n",
					   "Bandwidth: 1685936\r\nX-Accelerate-Streaming: AccelBandwidth=2147483647;AccelDuration=10000\r\n",
					   sizeof(cmd));
