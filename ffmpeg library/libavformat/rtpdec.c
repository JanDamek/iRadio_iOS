...
extern RTPDynamicProtocolHandler ff_ms_rtp_asf_pfv_handler,
								...
								ff_ms_rtp_asf_pfs_handler, /* AJS SUBTITLE HANDLING */
                                 ff_ms_rtp_wms_rtx_handler;
...
void av_register_rtp_dynamic_payload_handlers(void)
{
    ...
    ff_register_dynamic_payload_handler(&ff_ms_rtp_asf_pfs_handler); /* AJS SUBTITLE HANDLING */
    ff_register_dynamic_payload_handler(&ff_ms_rtp_wms_rtx_handler);
}
...
RTPDemuxContext *rtp_parse_open(AVFormatContext *s1, AVStream *st, URLContext *rtpc, int payload_type, RTPPayloadData *rtp_payload_data)
{
    ...
    if (!strcmp(ff_rtp_enc_name(payload_type), "MP2T")) {
        s->ts = mpegts_parse_open(s->tx.ic);
        if (s->ts == NULL) {
            av_free(s);
            return NULL;
        }
...
static void finalize_packet(RTPDemuxContext *s, AVPacket *pkt, uint32_t timestamp)
{
  ...
  // AJS //   pkt->stream_index = s->st->index;
}
